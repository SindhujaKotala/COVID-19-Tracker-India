...
#url = 'https://github.com/imdevskp/covid-19-india-data/blob/master/complete.csv'
...
def load_data():
    data = pd.read_csv(url)#, sep=';', error_bad_lines=False)#, parse_dates=[['date']])
    data = data.rename(columns={'Total Confirmed cases(Indian National)':'confirmed_cases_indian)', 'Total Confirmed cases( Foreign National )':'confirmed_cases_Foreign'}, inplace=True)
    #3data.rename(columns={'Total Confirmed cases(Indian National)':'Confirmed Cases(Indian)'}, inplace=True)
    data = data.rename(columns={'cured_discharged_migrated':'recovered'})
    data = data.rename(columns = {'Death':'death'})
    data = data.rename(columns = {'Date':'date'})
    data = data.rename(columns = {'Total Confirmed cases':'total_confirmed_cases'})
...
f = data1.nlargest(1)#np.argmax(data1['death'].values)
f = data1.get_value(f)
deaths = st.slider("Number of Deaths", 0, f)
st.write('As for,', d)
